...
def crawling(url,pre_gu_cortarName,dong_cortarName,dong_centerLat,dong_centerLon):
    ...
    try:
        ...
        if element.text == '해당되는 매물이 없습니다.' or element.text == '':
            ...
            # print('estate_tr_len',estate_tr_len)

            for estate_tr_row in range(1, estate_tr_len + 1):
                # print('estate_tr_row',estate_tr_row)
                ...
                # print('estate_th_len:',estate_th_len)
                if estate_th_len == 1:
                    estate_th_xpath = '/html/body/div[2]/div/section/div[2]/div[2]/div/div[2]/div[3]/div[1]/table/tbody/tr[{0}]/th'.format(
                        estate_tr_row)
                ...
                #                         print('estate_th:',estate_th)
                #                         print('estate_td:',estate_td)
                else:
...
if __name__ == "__main__" :
    ...
    for i in tqdm(range(len(gungu_result))):
        ...
        for j in range(len(dong_result)):
            ...
            complexNo_list = get_opst_list(dong_cortarNo[j])
            info_list = []
            ...
            for complexNo in complexNo_list:
                temp_url = 'https://new.land.naver.com/complexes/{0}?ms={1},{2},17&a=PRE:OPST&b=A1&e=RETAIL'.format(complexNo, dong_centerLat[j], dong_centerLon[j])
                info_list.append((temp_url,pre_gu_cortarName, dong_cortarName[j],dong_centerLat[j],dong_centerLon[j]))
            ...
            result_df.to_csv('./data/OPST/naver_selenium_OPST_multi({0}_{1}).csv'.format(gu_cortarName[i], dong_cortarName[j]))
            estate.to_csv('./data/OPST/naver_selenium_OPST_estate_multi({0}_{1}).csv'.format(gu_cortarName[i], dong_cortarName[j]))
